...
class Passphrase : Answer {
    override fun showAnswer(): String {
        ...
        return check(input).toString() + " -> " + checkStrict(input).toString()
    }

    fun check(lines: Array<String>): Int {
        return lines.filter { isValid(it) }.size
    }
    fun checkStrict(lines: Array<String>): Int {
        return lines.filter { isStrictValid(it) }.size
    }
    ...
    private fun isStrictValid(phrase: String): Boolean {
        val words = phrase.trim().split(Regex("\\s+")).map { it.toCharArray().sorted().toString() }
        return words.size == words.toSet().size
    }
}
